#define MPV_MCNew_1000_2000_cxx
#include "MPV_MCNew_1000_2000.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include <TRandom.h>
...
TH1D *hMCTrueELossUpstream = new TH1D("hMCTrueELossUpstream", "Energy loss prior to entering the TPC (MC Truth)", 250, 0, 250);
...
double TOFLowerZ = -52.5;
double TOFUpperZ = -57.5;

// #########################################
// ### Beam Halo Scintillator Dimensions ###
// #########################################
double HaloLowerX = 0;
double HaloUpperX = 0;
...
double HaloLowerZ = -62.5;
double HaloUpperZ = -64;

// ##########################################
// ### Rough Estimate of Steel Before TPC ###
// ##########################################
double RoughSteelLowerX = 0;
double RoughSteelUpperX = 0;

double RoughSteelLowerY = 0;
...
for (Long64_t jentry=0; jentry<nentries; jentry++) 
   ...
   // *** Downstream ***
   float LastSpacePointZ = 0;
   float LastSpacePointY = 999;
   float LastSpacePointX = 999;
	 
   float TempTrj2_X = 999;
   float TempTrj2_Y = 999;
   float TempTrj2_Z = 999;
   
   float RecoTPCTheta2 = 999;
   float RecoTPCPhi2   = 999;
   ...
   for(int nTPCtrk = 0; nTPCtrk < ntracks_reco; nTPCtrk++)
	 ...
	 // ### Recording this tracks downstream most X, Y, Z location, ###
	 // ###        which is inside the fiducial boundary            ###
	 if(trjPt_Z[nTPCtrk][ntrjpts] > LastSpacePointZ && trjPt_Y[nTPCtrk][ntrjpts] > YLowerFid && 
	    trjPt_Y[nTPCtrk][ntrjpts] < YUpperFid && trjPt_X[nTPCtrk][ntrjpts] > XLowerFid && 
	    trjPt_X[nTPCtrk][ntrjpts] < XUpperFid && trjPt_Z[nTPCtrk][ntrjpts] > trjPt_Z[nTPCtrk][ntrjpts-1])
	    {
	       
	    // ########################################
	    // ### Record the most downstream point ###
	    // ########################################
	    LastSpacePointZ = trjPt_Z[nTPCtrk][ntrjpts];
	    LastSpacePointY = trjPt_Y[nTPCtrk][ntrjpts];
	    LastSpacePointX = trjPt_X[nTPCtrk][ntrjpts];
	       
	    TempTrj2_X = pHat0_X[nTPCtrk][ntrjpts];
	    TempTrj2_Y = pHat0_Y[nTPCtrk][ntrjpts];
	    TempTrj2_Z = pHat0_Z[nTPCtrk][ntrjpts];
...
// ###########################################
TFile myfile("./ROOTFILES/MPV_MCNew_1000_2000.root","RECREATE");
